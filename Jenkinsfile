    ...
    stages{
        ...
        stage("Push Image to Nexus"){
            steps{
                ...
                sh 'docker login -u "$USERNAME" -p "$PASSWORD" nexus:8081'
                sh 'docker tag "$IMAGE_NAME":v1.0 localhost:8081/"$IMAGE_NAME":V1.0'
                sh 'docker push localhost:8081/"$IMAGE_NAME":v1.0'
                }
            }
        }
